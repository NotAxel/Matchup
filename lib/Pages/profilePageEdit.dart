...
class ProfilePageEdit extends StatefulWidget {
  ...
  @override
  _ProfilePageEdit createState() => _ProfilePageEdit ();
}

class _ProfilePageEdit extends State<ProfilePageEdit> {
  ...
  void initState() {
    _isLoading = false;
    _mainChar = widget.user.getMain;
    _secondaryChar = widget.user.getSecondary;
    _region = widget.user.getRegion;
    _skill = widget.user.getSkill;
    _nintendoID = widget.user.getFriendCode;
    super.initState();
  }
  ...
  Widget showNintendoFriendCodeEntryForm() {
    ...
    return new Center(
      child: TextFormField(
        key: Key("FriendCode"),
        initialValue: widget.user.getFriendCode,
        obscureText: false,
        maxLines: 1,
        style: style,
        autofocus: false,
  ...
  Widget showUserNameField() {
    return new Center(
      child: TextFormField(
        key: Key("Username"),
        initialValue: widget.user.getUserName,
        obscureText: false,
        maxLength: 30,
        maxLines: 1,
        style: style,
  ...
  validateAndSubmit() async {
    ...
    if (validateAndSave()){
      ...
      await widget.user.initializeData(currUser);
    }
    setState(() {
      _isLoading = false;
    });
  ...
  Widget build(BuildContext context) {
    ...
    _confirmer = ActionConfirmation(
      context, 
      "Are you sure you want to stop editing your account?",
      // confirm cancelation 
      () async {
